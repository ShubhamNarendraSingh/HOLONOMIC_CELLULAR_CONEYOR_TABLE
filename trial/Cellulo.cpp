...
void UNIT1(int Angle, int pwm) {
  int pi = 3.1412;
  Angle = (pi / 180) * Angle;
  int Vx = pwm * cos(Angle);
  int Vy = pwm * sin(Angle);
  int VA = Vx;
  int VB = -(Vx * (0.5)) - (Vy * (0.866));
  int VC = -(Vx * (0.5)) + (Vy * (0.866));
